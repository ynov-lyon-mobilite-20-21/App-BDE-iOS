//
//  ColorExtension.swift
//  App-BDE-iOS
//
//  Created by Nicolas Barbosa on 02/11/2020.
//

import SwiftUI

extension Color {
    static let blackToWhite = Color("blackToWhite")
    static let blueToWhite = Color("blueToWhite")
    static let greenToYellow = Color("greenToYellow")
    static let pinkToGreen = Color("pinkToGreen")
    static let pinkToYellow = Color("pinkToYellow")
    static let whiteToBlack = Color("whiteToBlack")
    static let whiteToBlue = Color("whiteToBlue")
    static let yellowToWhite = Color("yellowToWhite")
    static let whiteToPink = Color("whiteToPink")
    static let blueToBlack = Color("blueToBlack")
    static let whiteToYellow = Color("whiteToYellow")
    static let blackToGreen = Color("blackToGreen")
    static let bdeBlue = Color("bdeBlue")
    static let bdePink = Color("bdePink")
    static let bdeGreen = Color("bdeGreen")
    static let bdeYellow = Color("bdeYellow")
    static let blueToGreenGradiantStartingPoint = Color("blueToGreenGradiantStartingPoint")
    static let blueToGreenGradiantEndingPoint = Color("blueToGreenGradiantEndingPoint")
    static let bdeForm = Color("bdeForm")
}
