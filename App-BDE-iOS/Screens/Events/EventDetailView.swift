            ...
            VStack {
                GeometryReader { reader in
                    ZStack(alignment: Alignment(horizontal: .center, vertical: .top)) {
                        Image(vm.selectedItem.image.rawValue)
                            .resizable()
                            .aspectRatio(contentMode: .fill)
                            .matchedGeometryEffect(id: "image" + vm.selectedItem.id, in: animation)
                            .frame(width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height / 2.5)
                            .background(Color.blueToBlack)
                        HStack{
                            Spacer()
                            Button(action: {
                                withAnimation(.spring()){ vm.show.toggle() }
                            }) {
                ...
                HStack{
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        Text(vm.selectedItem.type.rawValue)
                            .fontWeight(.bold)
                ...
                .matchedGeometryEffect(id: "content" + vm.selectedItem.id, in: animation)
                .padding()
